...
function Ejemplos (num,title) {
	...
	switch(num){
		...
		case 7:
			...
			var str = MuestraEjemploBinomialNegativa();
		break;
	}
		var footer = `</div>
...
$('#CalculaBinomialNegativa').click(function(){	
	$('#Result').val(CalculaBinomialNegativa($('#k').val(),$('#n').val(),$('#p').val(),$('#q').val()));
	$('#ResultPorc').val(CalculaBinomialNegativa($('#k').val(),$('#n').val(),$('#p').val(),$('#q').val())*100 + "%");
})
$('#CalculaNormal').click(function(){	
	$('#Result').val("0.0749");
	$('#ResultPorc').val("7.49%");
})
$('#CalculaContinua').click(function(){	
	$('#Result').val("0.2591");
	$('#ResultPorc').val("25.91%");
})
...
function MuestraEjemploBinomialNegativa(){
	...
	return 			`<p class="pModal">
					   ...
					   <br>
					   <p class="pModal">
					   		Debido a que es una distribución binomial negativa, los no involucrados sólo pueden ser votantes o no votantes.
					   </p>
					   <br>					  					   
					   <div align="left">
					   <ul>
					   		<li>k = 3, Nos preguntan cual es la probabilidad de tener 3 "niños"</li>
					   		<li>n = 5, El espectro muestral, la familia tiene 4 hijos</li>
					   		<li>p = 0,61, La probabilidad de exito, 61% dado en decimales</li>
					   		<li>q = 0,39, La probabilidad de error, 39% dado en decimales</li>
					   </ul>
					   <p class="pModal">La formula que nos permite encontrar la probabilidad binomial negativa es: <img class="imgModal" src="img/FormulaBinomialNeg.PNG">
					    ahora bastara con sustituir los valores de la formula con los qué, ya hemos obtenido del ejericio.
					   </p>
					   <input type="number" class="form-control inputModal" id="k" placeholder="K" required>
					   <input type="number" class="form-control inputModal" id="n" placeholder="n" required>
					   ...
					   <p class="pModel">Nos piden P(X=3) = <input type="number" class="inputModalResult" id="Result" placeholder="Resultado">
					   <input type="text" class="inputModalResult" id="ResultPorc" placeholder="Porcentaje">
					   <button type="button" id="CalculaBinomialNegativa" class="btn btn-success">Calcular</button>
					  </p>
					   </div>
					   `;
}
function MuestraEjemploNormal(){
	...
	return 			`<p class="pModal">
					   ...
					   <ul>
					   		<li>w = 185, p<185</li>
					   		<li>uw = 167, media de las estaturas de los colombianos en cm</li>
					   		<li>r = 12,5, desviacion estandar de las estaturas</li>					   		
					   </ul>
					   <p class="pModal">La formula que nos permite encontrar la probabilidad normal es: <img class="imgModal" src="img/FormulaNormal.PNG">
					    ahora bastara con sustituir los valores de la formula con los qué, ya hemos obtenido del ejericio.
					   </p>
					   <input type="number" class="form-control inputModal" id="k" placeholder="w" required>
					   <input type="number" class="form-control inputModal" id="n" placeholder="uw" required>
					   <input type="number" class="form-control inputModal" id="p" placeholder="r" required>					   
					   <p class="pModel">Nos piden P(X<185) = <input type="number" class="inputModalResult" id="Result" placeholder="Resultado">
					   <input type="text" class="inputModalResult" id="ResultPorc" placeholder="Porcentaje">
					   <button type="button" id="CalculaNormal" class="btn btn-success">Calcular</button>
					  </p>
					   </div>
					   `;
}
...
function MuestraContinua(){
	...
	return 			`<p class="pModal">
					   ...
					   <div align="left">
					   <ul>
					   		<li>landa (l) = 1/10000 = 0.0001 horas de vida promedio</li>					   	
					   		<li>x = 3000</li>					   		
					   </ul>
					   <p class="pModal">La formula que nos permite encontrar la probabilidad continua es: <img class="imgModal" src="img/continua.PNG">
					    ahora bastara con sustituir los valores de la formula con los qué, ya hemos obtenido del ejericio.
					   </p>
					   <input type="number" class="form-control inputModal" id="k" placeholder="l" required>				   
					   <input type="number" class="form-control inputModal" id="p" placeholder="x" required>					   
					   <p class="pModel">Nos piden P(X<=3000) = <input type="number" class="inputModalResult" id="Result" placeholder="Resultado">
					   <input type="text" class="inputModalResult" id="ResultPorc" placeholder="Porcentaje">
					   <button type="button" id="CalculaContinua" class="btn btn-success">Calcular</button>
					  </p>
					   </div>
					   `;
}
...
function CalculaBinomialNegativa(x,y,p,q){	
	var n = Number(x)+Number(y)-1;
	var k = Number(y)-1; 	
	return combinatoria(n,k)*Math.pow(p,y)*Math.pow(q,x);
}
